...
Optimaized runtime: 31.88759896794022
R=3.4300992648830486
Could not reach to under 30s.
'''
"""
    N-body simulation.
"""
...
def update_vs(v1, v2, dt, dx, dy, dz, m1, m2):
    ...
    v1[0] -= dx * (m2)*compute_m#compute_b(m2, dt, dx, dy, dz)
    v1[1] -= dy * (m2)*compute_m#compute_b(m2, dt, dx, dy, dz)
    v1[2] -= dz * (m2)*compute_m#compute_b(m2, dt, dx, dy, dz)
    v2[0] += dx * (m1)*compute_m#compute_b(m1, dt, dx, dy, dz)
    v2[1] += dy * (m1)*compute_m#compute_b(m1, dt, dx, dy, dz)
    v2[2] += dz * (m1)*compute_m#compute_b(m1, dt, dx, dy, dz)
...
def nbody(loops, reference, iterations,BODIES,liter):
...
if __name__ == '__main__':   
    nbody(100, 'sun', 20000,BODIES,liter)

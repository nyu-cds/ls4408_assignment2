...
Optimaized runtime: 104.60501941530674
R=1.06276334726628
...
def advance(dt):
    ...
    bodys=BODIES
    seenit = set()
    for body1 in bodys.keys():
        for body2 in bodys.keys():
            if (body1 != body2) and not (body2 in seenit):
                ([x1, y1, z1], v1, m1) = bodys[body1]
                ([x2, y2, z2], v2, m2) = bodys[body2]
                (dx, dy, dz) = compute_deltas(x1, x2, y1, y2, z1, z2)
                update_vs(v1, v2, dt, dx, dy, dz, m1, m2)
                seenit.add(body1)
        
    for body in bodys.keys():
        (r, [vx, vy, vz], m) = bodys[body]
